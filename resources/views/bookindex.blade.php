    ...
    <body>
      <div class="container">
        <div class="starter-template">
          ...
          <a href="/flipbooks/create">
            <button>Create Flipbook</button>
          </a>
        </div>
        <div class="row"> @foreach($flipbooks as $fb) <div class="col-md-2">
            ...
            <a href="{{ route('flipbook.edit',$fb->id) }}" style="float: left;clear: both;width:100%;margin-top: 10px;">
              <button id="add_files" class="btn btn-medium btn-general input-block-level" type="submit" style="width:100%;">Edit</button>
            </a>
            <form method="POST" action="{{ route('flipbook.destroy', $fb->id) }}">
                @csrf  @method('DELETE')  <button id="delete_btn_{{ $fb->id }}" class="btn btn-danger btn-sm" type="submit">Delete</button>
            </form>
          </div> @endforeach </div>
      </div>
      ...
      <script>
        window.jQuery || document.write(' < script src = "{{ asset('../turnjs/js/jquery.min.js') }}" > < \/script>')
      </script>
